(function() {
  ...
  function renderProjects(d) {
    ...
    var cardList = projs.map(function (p) {
    ...
    var cards = cardList.join('');

    var body = cards;
    if (projs.some(function (x) { return x.group; })) {
      var order = [
        { key: 'independent', label: 'Independent, Academic & Open Source', icon: 'rocket_launch' },
        { key: 'company', label: 'Company & Client Work', icon: 'apartment' }
      ];
      var byGroup = projs.map(function (x, i) { return { g: x.group || 'company', html: cardList[i] }; });
      body = order.map(function (grp) {
        var mine = byGroup.filter(function (x) { return x.g === grp.key; });
        if (!mine.length) return '';
        return '<div class="group-header ic proj-sec-head" style="margin:18px 4px 6px">' +
            '<span class="material-symbols-rounded">' + grp.icon + '</span>' +
            '<span class="title">' + grp.label + '</span><span class="bar"></span></div>' +
          mine.map(function (x) { return x.html; }).join('');
      }).join('');
    }
  ...
  function renderResumeView(d, content) {
    ...
    } else if (content === 'tmobile' && r.tmobile) {
    ...
    } else if (content === 'moringfde' && r.moringfde) {
      expHtml = renderResumeCuratedExperience(r.moringfde, true);
  ...
  var RESUME_CONTENTS = ['highlights', 'mercury', 'simplifiedq', 'amex', 'bcg', 'nvidia', 'google', 'bain', 'tmobile', 'amexclassic', 'onepage', 'top5', 'complete', 'anthropiclabs', 'anthropicarch', 'openaicodex', 'openaifde', 'ppxagents', 'ppxfde', 'startup', 'googlefde', 'appledata', 'applefde', 'appleai', 'applecombo', 'moringfde'];
  ...
  (function () {
    ...
    // Brand marks. `c` is a CSS token, never a literal — the platform hex values
    // live in portfolio.css :root as --brand-<key>, so this renderer (and data.js)
    // stay hex-free and the marks track light/dark/print like every other token.
    var B = {
      hope:       { c: 'var(--accent-orange)', n: 'Hope', i: '<path d="M12 2.5l2.09 5.32L19.5 9.9l-4.2 3.66 1.2 5.44L12 16.2l-4.5 2.8 1.2-5.44L4.5 9.9l5.41-2.08L12 2.5z"/>' },
      youtube:    { c: 'var(--brand-youtube)', i: '<path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/>' },
      vimeo:      { c: 'var(--brand-vimeo)', i: '<path d="M23.9765 6.4168c-.105 2.338-1.739 5.5429-4.894 9.6088-3.2679 4.247-6.0258 6.3699-8.2898 6.3699-1.409 0-2.578-1.294-3.553-3.881l-1.9179-7.1138c-.719-2.584-1.488-3.878-2.312-3.878-.179 0-.806.378-1.881 1.132L0 7.3008c1.185-1.042 2.351-2.084 3.501-3.128C5.08 2.8169 6.266 2.0769 7.055 2.0049c1.867-.18 3.016 1.1 3.447 3.838.465 2.953.789 4.789.971 5.5069.539 2.45 1.131 3.674 1.776 3.674.502 0 1.256-.794 2.265-2.385 1.004-1.589 1.54-2.798 1.612-3.628.144-1.371-.395-2.061-1.614-2.061-.574 0-1.167.121-1.777.391 1.186-3.868 3.434-5.757 6.762-5.637 2.473.06 3.628 1.664 3.493 4.797z"/>' },
      spotify:    { c: 'var(--brand-spotify)' },
      soundcloud: { c: 'var(--brand-soundcloud)' },
      applemusic: { c: 'var(--brand-applemusic)' },
      figma:      { c: 'var(--brand-figma)' },
      codepen:    { c: 'var(--brand-codepen)' },
      loom:       { c: 'var(--brand-loom)' },
      bluesky:    { c: 'var(--brand-bluesky)' },
      linkedin:   { c: 'var(--brand-linkedin)', i: '<path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>' },
      substack:   { c: 'var(--brand-substack)' },
      flickr:     { c: 'var(--brand-flickr)' },
      tiktok:     { c: 'var(--brand-tiktok)' },
      instagram:  { c: 'var(--brand-instagram)', i: '<path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 1 0 0 12.324 6.162 6.162 0 0 0 0-12.324zM12 16a4 4 0 1 1 0-8 4 4 0 0 1 0 8zm6.406-11.845a1.44 1.44 0 1 0 0 2.881 1.44 1.44 0 0 0 0-2.881z"/>' },
      x:          { c: 'var(--brand-x)', i: '<path d="M14.234 10.162 22.977 0h-2.072l-7.591 8.824L7.251 0H.258l9.168 13.343L.258 24H2.33l8.016-9.318L16.749 24h6.993zm-2.837 3.299-.929-1.329L3.076 1.56h3.182l5.965 8.532.929 1.329 7.754 11.09h-3.182z"/>' },
      threads:    { c: 'var(--brand-threads)' },
      pinterest:  { c: 'var(--brand-pinterest)' },
      dribbble:   { c: 'var(--brand-dribbble)', i: '<path d="M12 24C5.385 24 0 18.615 0 12S5.385 0 12 0s12 5.385 12 12-5.385 12-12 12zm10.12-10.358c-.35-.11-3.17-.953-6.384-.438 1.34 3.684 1.887 6.684 1.992 7.308 2.3-1.555 3.936-4.02 4.395-6.87zm-6.115 7.808c-.153-.9-.75-4.032-2.19-7.77l-.066.02c-5.79 2.015-7.86 6.025-8.04 6.4 1.73 1.358 3.92 2.166 6.29 2.166 1.42 0 2.77-.29 4-.814zm-11.62-2.58c.232-.4 3.045-5.055 8.332-6.765.135-.045.27-.084.405-.12-.26-.585-.54-1.167-.832-1.74C7.17 11.775 2.206 11.71 1.756 11.7l-.004.312c0 2.633.998 5.037 2.634 6.855zm-2.42-8.955c.46.008 4.683.026 9.477-1.248-1.698-3.018-3.53-5.558-3.8-5.928-2.868 1.35-5.01 3.99-5.676 7.17zM9.6 2.052c.282.38 2.145 2.914 3.822 6 3.645-1.365 5.19-3.44 5.373-3.702-1.81-1.61-4.19-2.586-6.795-2.586-.825 0-1.63.1-2.4.285zm10.335 3.483c-.218.29-1.935 2.493-5.724 4.04.24.49.47.985.68 1.486.08.18.15.36.22.53 3.41-.43 6.8.26 7.14.33-.02-2.42-.88-4.64-2.31-6.38z"/>' },
      behance:    { c: 'var(--brand-behance)', i: '<path d="M16.969 16.927a2.561 2.561 0 0 0 1.901.677 2.501 2.501 0 0 0 1.531-.475c.362-.235.636-.584.779-.99h2.585a5.091 5.091 0 0 1-1.9 2.896 5.292 5.292 0 0 1-3.091.88 5.839 5.839 0 0 1-2.284-.433 4.871 4.871 0 0 1-1.723-1.211 5.657 5.657 0 0 1-1.08-1.874 7.057 7.057 0 0 1-.383-2.393c-.005-.8.129-1.595.396-2.349a5.313 5.313 0 0 1 5.088-3.604 4.87 4.87 0 0 1 2.376.563c.661.362 1.231.87 1.668 1.485a6.2 6.2 0 0 1 .943 2.133c.194.821.263 1.666.205 2.508h-7.699c-.063.79.184 1.574.688 2.187ZM6.947 4.084a8.065 8.065 0 0 1 1.928.198 4.29 4.29 0 0 1 1.49.638c.418.303.748.711.958 1.182.241.579.357 1.203.341 1.83a3.506 3.506 0 0 1-.506 1.961 3.726 3.726 0 0 1-1.503 1.287 3.588 3.588 0 0 1 2.027 1.437c.464.747.697 1.615.67 2.494a4.593 4.593 0 0 1-.423 2.032 3.945 3.945 0 0 1-1.163 1.413 5.114 5.114 0 0 1-1.683.807 7.135 7.135 0 0 1-1.928.259H0V4.084h6.947Zm-.235 12.9c.308.004.616-.029.916-.099a2.18 2.18 0 0 0 .766-.332c.228-.158.411-.371.534-.619.142-.317.208-.663.191-1.009a2.08 2.08 0 0 0-.642-1.715 2.618 2.618 0 0 0-1.696-.505h-3.54v4.279h3.471Zm13.635-5.967a2.13 2.13 0 0 0-1.654-.619 2.336 2.336 0 0 0-1.163.259 2.474 2.474 0 0 0-.738.62 2.359 2.359 0 0 0-.396.792c-.074.239-.12.485-.137.734h4.769a3.239 3.239 0 0 0-.679-1.785l-.002-.001Zm-13.813-.648a2.254 2.254 0 0 0 1.423-.433c.399-.355.607-.88.56-1.413a1.916 1.916 0 0 0-.178-.891 1.298 1.298 0 0 0-.495-.533 1.851 1.851 0 0 0-.711-.274 3.966 3.966 0 0 0-.835-.073H3.241v3.631h3.293v-.014ZM21.62 5.122h-5.976v1.527h5.976V5.122Z"/>' },
      medium:     { c: 'var(--brand-medium)', i: '<path d="M13.54 12a6.8 6.8 0 0 1-6.77 6.82A6.8 6.8 0 0 1 0 12a6.8 6.8 0 0 1 6.77-6.82A6.8 6.8 0 0 1 13.54 12zm7.42 0c0 3.54-1.51 6.42-3.38 6.42-1.87 0-3.39-2.88-3.39-6.42s1.52-6.42 3.39-6.42 3.38 2.88 3.38 6.42zM24 12c0 3.17-.53 5.75-1.19 5.75-.66 0-1.19-2.58-1.19-5.75s.53-5.75 1.19-5.75C23.47 6.25 24 8.83 24 12z"/>' },
      gist:       { c: 'var(--brand-gist)', i: '<path d="M12 .297c-6.63 0-12 5.373-12 12 0 5.303 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61C4.422 18.07 3.633 17.7 3.633 17.7c-1.087-.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 22.092 24 17.592 24 12.297c0-6.627-5.373-12-12-12"/>' },
      link:       { c: 'var(--brand-link)', i: '<path d="M11.99 2C6.47 2 2 6.48 2 12s4.47 10 9.99 10C17.52 22 22 17.52 22 12S17.52 2 11.99 2zm6.93 6h-2.95c-.32-1.25-.78-2.45-1.38-3.56 1.84.63 3.37 1.91 4.33 3.56zM12 4.04c.83 1.2 1.48 2.53 1.91 3.96h-3.82c.43-1.43 1.08-2.76 1.91-3.96zM4.26 14C4.1 13.36 4 12.69 4 12s.1-1.36.26-2h3.38c-.08.66-.14 1.32-.14 2 0 .68.06 1.34.14 2H4.26zm.82 2h2.95c.32 1.25.78 2.45 1.38 3.56-1.84-.63-3.37-1.9-4.33-3.56zm2.95-8H5.08c.96-1.66 2.49-2.93 4.33-3.56C8.81 5.55 8.35 6.75 8.03 8zM12 19.96c-.83-1.2-1.48-2.53-1.91-3.96h3.82c-.43 1.43-1.08 2.76-1.91 3.96zM14.34 14H9.66c-.09-.66-.16-1.32-.16-2 0-.68.07-1.35.16-2h4.68c.09.65.16 1.32.16 2 0 .68-.07 1.34-.16 2zm.25 5.56c.6-1.11 1.06-2.31 1.38-3.56h2.95c-.96 1.65-2.49 2.93-4.33 3.56zM16.36 14c.08-.66.14-1.32.14-2 0-.68-.06-1.34-.14-2h3.38c.16.64.26 1.31.26 2s-.1 1.36-.26 2h-3.38z"/>' }
    };
    ...
    // ── RICH POST CARD ───────────────────────────────────────────────────
    // Why this exists: LinkedIn only serves /embed/feed/update/<urn> for posts
    // whose AUTHOR opted that specific post into embedding. Every other post
    // returns a signed-out wall inside the iframe ("Sign in to see … / This post
    // is unavailable") — nothing we can do from this side, and no amount of
    // retrying fixes it. Even the posts that DO embed render as an unclamped
    // wall of text with the image and the engagement counts pushed below the
    // iframe's fold.
    // So when an entry carries its own post payload (text / image / counts) we
    // stop asking the platform and render the post ourselves: a YouTube-style
    // rich preview — author + headline + age, brand mark, CSS-clamped text, a
    // LOCAL image, engagement counts, and exactly one outbound link. Fully
    // self-contained: no third-party request, no auth, no layout shift, works
    // offline and from file://.
    function isPostCard(p) {
      return !!(p && (p.text || p.image || p.reactions != null || p.comments != null));
    }
    function postCounts(post) {
      var out = [];
      if (post.reactions == null && post.comments == null && post.meta) return String(post.meta);
      if (post.reactions != null && post.reactions !== '') out.push(post.reactions + ' ' + plural(Number(post.reactions), 'reaction'));
      if (post.comments != null && post.comments !== '') out.push(post.comments + ' ' + plural(Number(post.comments), 'comment'));
      return out.join(' · ');
    }
    function buildPostCard(post, key, cfg) {
      var d = window.HOPE_DATA || {};
      var url = String(post.url || '#');
      var name = cfg.name;
      var author = String(post.author || (d.meta && d.meta.name) || '');
      var avatar = String((post.avatar != null ? post.avatar : (d.identity && d.identity.photo)) || '');
      var hl = String(post.author_headline || '');
      var b = B[key] || B.link;
      // Blank lines are dead weight inside a 4-line clamp — collapse runs of
      // them to a single break. The clamp itself is CSS (-webkit-line-clamp),
      // never a JS substring, so the preview can never cut mid-word.
      var body = String(post.text || post.caption || '').replace(/\r/g, '').replace(/\n{2,}/g, '\n').trim();
      var counts = postCounts(post);

      var html = '<div class="post-head">'
        + (avatar ? '<img class="post-avatar" src="' + esc(avatar) + '" alt="' + esc(author) + '" loading="lazy" decoding="async">'
                  : '<span class="post-avatar post-avatar-fallback" aria-hidden="true">' + esc((author || '?').charAt(0)) + '</span>')
        + '<span class="post-ident">'
        +   (author ? '<span class="post-author">' + esc(author) + '</span>' : '')
        +   (hl ? '<span class="post-headline">' + esc(hl) + '</span>' : '')
        +   (post.posted ? '<span class="post-age">' + esc(String(post.posted)) + '</span>' : '')
        + '</span>'
        + (b.i ? '<span class="post-mark" aria-hidden="true"><svg viewBox="0 0 24 24" fill="currentColor">' + b.i + '</svg></span>' : '')
        + '</div>';
      if (body) html += '<p class="post-text">' + esc(body) + '</p>';
      // The media is a second route to the same destination as the CTA below —
      // hidden from AT so the card exposes one link, not two.
      if (post.image) {
        html += (post.deep
            ? '<a class="post-media" href="#" data-deep="' + esc(String(post.deep)) + '" tabindex="-1" aria-hidden="true">'
            : '<a class="post-media" href="' + esc(url) + '" target="_blank" rel="noopener" tabindex="-1" aria-hidden="true">')
          + '<img src="' + esc(String(post.image)) + '" alt="' + esc(String(post.image_alt || '')) + '" loading="lazy" decoding="async"></a>';
      }
      var hasUrl = url && url !== '#';
      var cta = hasUrl
        ? '<a class="post-cta" href="' + esc(url) + '" target="_blank" rel="noopener">'
          + esc(post.title || ('View on ' + name))
          + '<span class="material-symbols-rounded ext">arrow_outward</span></a>'
        : '';
      html += '<div class="post-foot">'
        + '<span class="post-counts">' + esc(counts) + '</span>'
        + cta
        + '</div>';
      if (post.deep) {
        html += '<a class="post-deep" href="#" data-deep="' + esc(String(post.deep)) + '">'
          + esc(post.deep_label || 'View the related experience')
          + '<span class="material-symbols-rounded ext">arrow_forward</span></a>';
      }

      var card = document.createElement('article');
      card.className = 'social-card social-' + key + ' social-cls-post';
      card.innerHTML = html;
      return card;
    }
    ...
    function buildSocialCard(post) {
      ...
      var url = String(post.url);
      var name = cfg.name;
      // Entry carries its own post payload → render OUR card, never the
      // platform iframe (see buildPostCard for why).
      if (isPostCard(post)) return buildPostCard(post, key, cfg);
    ...
    function isEmbeddable(p) {
      ...
      // A self-rendered post card is always featurable — it needs no platform
      // embed at all, so it qualifies for the Overview strip and stays out of
      // the contact-row profile pills.
      if (isPostCard(p)) return true;
